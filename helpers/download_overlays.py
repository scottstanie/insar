...
def download(idx, overlay, look):
    ...
    subprocess.call(
        "scp lidar.csr.utexas.edu:%s ./%s" % (overlay, overlay_numbered), shell=True
    )
    subprocess.call(
        "scp lidar.csr.utexas.edu:%s ./%s" % (look, look_numbered), shell=True
    )
...
if __name__ == "__main__":
    ...
    quick_looks = [
        line.replace("map-overlay.kml", "quick-look.png") for line in overlays
    ]
    with ThreadPoolExecutor(max_workers=10) as executor:
        procs = []
        for idx, overlay, look in zip(
            range(1, len(overlays) + 1), overlays, quick_looks
        ):
