...
leg=ax.legend([DATNS[0],DAT[0],LS[0],LS0[0],TSIA[0],MIDAS[0],TRUTH[0]],['Data (No Steps)',
           'Data     $\hat{m}$        $s_{\hat{m}}$ (mm/yr)',
           'LS:       '+str('%.2f' % (LSout[0]*12))+'  '+str('%.3f' % (12*LSerr)),
           'LSNI:    '+str('%.2f' % (LS0out[0]*12))+'  '+str('%.3f' % (12*LS0err)),
           'TSIA:    '+str('%.2f' % (TSIAout[0]*12))+'  '+str('%.3f' % (12*TSIAout[3])),
           'MIDAS: '+str('%.2f' % (MIDASout[0]*12))+'  '+str('%.3f' % (12*MIDASout[3])),
           'Truth:   '+str('%.2f' % 10)])
...
plt.title('Linear Trend (10mm/yr) + Annual Sinusoid (2mm/yr)\n + Noise ($\sigma$=1.5mm) + Steps at 3 & 5 Years')
